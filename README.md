...
| bool | bool GetFromInventory(Item* p_item) | 从仓库中获取物品到背包中，可能发生交换 |
| void | MoveToInventory(Item* p_item) | 将item从背包移入到仓库中 |
...
| Return | Name | Description |
| - | - | - |
| Item* | Create(ItemType type, int id) | 根据type和id生产item |
| Item* | Create(string name) | 根据name生产item |
| ItemMixStat* | GetItemMixedStat(string name) | 返回该item的合成表 |
